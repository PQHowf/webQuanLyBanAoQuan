package Controller;

import java.io.IOException;
import java.util.List;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import HoaDonModel.ChiTietHoaDon;
import HoaDonModel.ChiTietHoaDonDAO;

/**
 * Servlet implementation class HienThiChiTietHoaDon
 */
@WebServlet("/HienThiChiTietHoaDon")
public class HienThiChiTietHoaDon extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HienThiChiTietHoaDon() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			ChiTietHoaDonDAO cthddao = new ChiTietHoaDonDAO();
			String maHoaDonStr = request.getParameter("maHoaDon");
			long maHoaDon = Long.parseLong(maHoaDonStr);
			List<ChiTietHoaDon> dsChiTiet = cthddao.LayThongTinHoaDon(maHoaDon);
			request.setAttribute("dsChiTiet", dsChiTiet);
	        request.getRequestDispatcher("HienThiChiTietHoaDon.jsp").forward(request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
		
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
	}

}
